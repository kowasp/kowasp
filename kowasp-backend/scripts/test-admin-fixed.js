...
async function testAdminFeatures() {
  console.log('🧪 Testing KOWASP Admin Features (Fixed)\n');
  ...
  try {
    ...
    // First login as a regular user (using existing test user)
    const regularUserLoginResponse = await fetch(`${BASE_URL}/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
    ...
    if (regularUserLoginResponse.ok) {
      const regularUserData = await regularUserLoginResponse.json();
      ...
      if (regularUserAdminResponse.status === 403) {
        console.log('✅ Regular user properly blocked from admin endpoints (403 Forbidden)');
      } else if (regularUserAdminResponse.status === 401) {
        console.log('⚠️  Regular user blocked due to authentication issue (401 Unauthorized)');
      } else {
        console.log('❌ Regular user should be blocked from admin endpoints');
      }
    } else {
      console.log('⚠️  Could not test regular user access - login failed');
    }
