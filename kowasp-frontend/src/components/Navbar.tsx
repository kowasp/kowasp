  ...
  return (
    <nav className="sticky top-0 z-40 bg-white/70 backdrop-blur-md shadow-lg rounded-b-2xl px-4 py-2 flex items-center justify-between mb-8 border-b border-gray-100">
      <div className="flex items-center gap-6">
        <Link href="/dashboard" className="text-2xl font-extrabold tracking-tight text-black hover:text-gray-800 transition-colors">
          KOWASP
        </Link>
        <div className="hidden lg:flex gap-4 ml-8">
          <Link href="/dashboard" className="px-3 py-2 rounded-lg text-black hover:bg-blue-50 hover:text-blue-700 font-medium transition-colors">Dashboard</Link>
          ...
          {user?.role === 'admin' && (
            <Link href="/admin/dashboard" className="px-3 py-2 rounded-lg text-black hover:bg-blue-50 hover:text-blue-700 font-medium transition-colors">Admin</Link>
          )}
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-white/95 backdrop-blur-md shadow-lg rounded-b-2xl z-50 border-b border-gray-100 lg:hidden animate-fade-in">
          <ul className="flex flex-col gap-2 p-4">
            <li><Link href="/dashboard" onClick={() => setMenuOpen(false)} className="block px-3 py-2 rounded-lg text-black hover:bg-blue-50 hover:text-blue-700 font-medium transition-colors">Dashboard</Link></li>
            ...
            {user?.role === 'admin' && <li><Link href="/admin/dashboard" onClick={() => setMenuOpen(false)} className="block px-3 py-2 rounded-lg text-black hover:bg-blue-50 hover:text-blue-700 font-medium transition-colors">Admin</Link></li>}
